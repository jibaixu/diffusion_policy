...
# 需要尝试的naction_steps超参数列表
NACTION_STEPS_LIST = [2, 3, 4, 5, 6, 7, 8, 9, 10]
...
class EvalConfig:
    ...
    save_path: str = f"outputs/test/AllTasks-v3/{ROBOT}/epoch_{EPOCH}"
    shader: str = "default"
    num_per_task: int = 20
...
def main(args: EvalConfig):
    ...
    # 记录所有naction_steps下的结果
    all_results = {}

    for naction_steps in NACTION_STEPS_LIST:
        print(f"\nEvaluating naction_steps={naction_steps}")
        # 记录每个naction_steps下每个任务的成功率
        total_successes = 0.0
        success_dict = {}
            ...
            for seed in range(args.num_per_task):
                ...
                with torch.inference_mode():
                    ...
                    with profiler.profile("env.step", total_steps=N, num_envs=num_envs):
                        for i in range(N):
                            ...
                            if len(action_queue) <= 0:
                                ...
                                # 取整个动作序列，最大长度为10
                                action_pred = action_dict.get('action_pred', None)
                                if action_pred is not None:
                                    action_chunk = action_pred.cpu().numpy()[0][:naction_steps]
                                else:
                                    # 兼容旧模型
                                    action_chunk = action_dict['action'].cpu().numpy()[0][:naction_steps]
                    ...
                    if args.save_video:
                        ...
                        del images
            env.close()
            print(f"Task {env_id} Success Rate (naction_steps={naction_steps}): {task_successes / args.num_per_task}")
            success_dict[env_id] = task_successes / args.num_per_task
        print(f"Total Success Rate (naction_steps={naction_steps}): {total_successes / (args.num_per_task * len(BENCHMARK_ENVS))}")
        success_dict['total_success'] = total_successes / (args.num_per_task * len(BENCHMARK_ENVS))
        all_results[naction_steps] = success_dict

    # 输出所有naction_steps下的成功率
    print("\n==== All naction_steps Results ====")
    for nstep, res in all_results.items():
        print(f"naction_steps={nstep}: {res}")
    # 保存到json
    with open(f"{os.path.dirname(args.save_path)}/success_dict_epoch_{EPOCH}_naction_steps.json", "w") as f:
